...
public class InviteFriends {
    public int inviteMostFriends(List<int[]> friendList, int num) {
        ...
        Set<Integer> remainingFriends = new HashSet<>();
        for(int i = 1; i <= num; i++) {
            remainingFriends.add(i);
        }

        return helper(friendConnections, remainingFriends);
    }

    public int helper(HashMap<Integer, List<Integer>> friendConnections, Set<Integer> remainingFriends) {
        if(remainingFriends.size() <= 1) {
            return remainingFriends.size();
        }
        int maxCount = 0;
        for(Integer user: remainingFriends) {
            Set<Integer> copiedSets = new HashSet<>(remainingFriends);
            copiedSets.remove(user);
            if(friendConnections.containsKey(user)) {
                for (Integer friend : friendConnections.get(user)) {
                    copiedSets.remove(friend);
                }
            }
            maxCount = Math.max(maxCount, helper(friendConnections, copiedSets) + 1);
        }
        return maxCount;
    }

    public static void main(String[] args) {
        InviteFriends solution = new InviteFriends();
